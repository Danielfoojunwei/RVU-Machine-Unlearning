"""Smoke tests for the BIPIA benchmark runner.
...
_CONFIGS_AVAILABLE = (
    (_PROJECT_ROOT / "configs" / "bipia.yaml").is_file()
...
class TestBIPIASmoke:
    """Smoke test suite for BIPIARunner."""

    def test_runner_instantiation(self, tmp_path: Path) -> None:
        """BIPIARunner can be instantiated with valid config."""
        from rvu.harness.bipia_runner import BIPIARunner

        runner = BIPIARunner(
            model_key="model_a",
            defense_name="rvu",
            config_path=str(_PROJECT_ROOT / "configs" / "bipia.yaml"),
            logdir=str(tmp_path / "bipia_smoke"),
        )
    ...
    def test_smoke_run_produces_output(self, tmp_path: Path) -> None:
        ...
        logdir = tmp_path / "bipia_smoke_run"
        runner = BIPIARunner(
            model_key="model_a",
            defense_name="rvu",
            config_path=str(_PROJECT_ROOT / "configs" / "bipia.yaml"),
    ...
    def test_metrics_conform_to_schema(self, tmp_path: Path) -> None:
        ...
        logdir = tmp_path / "bipia_schema_test"
        runner = BIPIARunner(
            model_key="model_a",
            defense_name="rvu",
            config_path=str(_PROJECT_ROOT / "configs" / "bipia.yaml"),
        ...
        metrics.setdefault("benchmark", "bipia")
