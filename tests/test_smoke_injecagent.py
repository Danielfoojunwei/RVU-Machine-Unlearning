"""Smoke tests for the InjecAgent benchmark runner.
...
_CONFIGS_AVAILABLE = (
    (_PROJECT_ROOT / "configs" / "injecagent.yaml").is_file()
...
class TestInjecAgentSmoke:
    """Smoke test suite for InjecAgentRunner."""

    def test_runner_instantiation(self, tmp_path: Path) -> None:
        """InjecAgentRunner can be instantiated with valid config."""
        from rvu.harness.injecagent_runner import InjecAgentRunner

        runner = InjecAgentRunner(
            model_key="model_a",
            defense_name="rvu",
            config_path=str(_PROJECT_ROOT / "configs" / "injecagent.yaml"),
            logdir=str(tmp_path / "injecagent_smoke"),
        )
    ...
    def test_smoke_run_produces_output(self, tmp_path: Path) -> None:
        ...
        logdir = tmp_path / "injecagent_smoke_run"
        runner = InjecAgentRunner(
            model_key="model_a",
            defense_name="rvu",
            config_path=str(_PROJECT_ROOT / "configs" / "injecagent.yaml"),
    ...
    def test_metrics_conform_to_schema(self, tmp_path: Path) -> None:
        ...
        logdir = tmp_path / "injecagent_schema_test"
        runner = InjecAgentRunner(
            model_key="model_a",
            defense_name="rvu",
            config_path=str(_PROJECT_ROOT / "configs" / "injecagent.yaml"),
        ...
        metrics.setdefault("benchmark", "injecagent")
